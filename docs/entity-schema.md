...
| content_type | enum | `text`, `image`, `voice`, `product_card`, `order_card`, `wishlist_item`, `wishlist_summary`, `summary`, `system` |
...
product_card:     { product_id: string, name: string, price_min?: number, price_max?: number, image_url: string }
order_card:       { order_id: string, status: string, eta?: string }
wishlist_item:    { wishlist_item_id: string, name: string, photo_url?: string, price_cny?: number, price_aud_estimate?: number, quantity: number, notes?: string }
wishlist_summary: { items: WishlistItem[], subtotal_aud_estimate: number, shipping_estimate_aud: number, total_estimate_aud: number, item_count: number }
...
Represents an item a client spotted and wants to remember during their Foshan shopping trip. Created from a photo + memo in the conversation. The primary in-trip shopping tool.
...
| name | string | client's label for the item (e.g. "Grey sofa at Factory X") |
| notes | text | optional; memo about the item |
| photo_urls | string[] | optional; one or more photos taken in-store |
| price_cny | integer | optional; price in CNY as discussed with the shop |
| price_aud_estimate | integer | optional; AUD cents estimate (converted by AI or agent) |
| quantity | integer | default 1 |
| status | enum | `considering`, `decided`, `removed` |

### Business Rules
- `quantity` must be at least 1
- At least one of `name`, `notes`, or `photo_urls` must be present
- `price_cny` and `price_aud_estimate` are both optional — a wishlist item can be added with just a photo
- Status transitions: `considering` → `decided` (client confirms they want it) or `considering` → `removed`
- Removed items are retained for history but excluded from wishlist summaries by default
